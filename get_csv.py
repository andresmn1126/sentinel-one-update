...
def export_agents_to_csv():
    ...
    with open(f'./downloads/CMS-CO_S1_Master_{now}.csv', 'w') as org:
        org.write(res.text)
    return res.text


def format_scan():
    with open(f'./downloads/CMS-CO_S1_Master_{now}.csv', 'r') as org:
        reader = csv.reader(org)

        with open(f'./downloads/CMS-CO_S1_ScanStatus_{now}.csv', 'w') as modified:
